...
if (isset($_POST['add-Item']) && $_POST['add-Item'] == 'true'):
	...
	elseif (!isset($_POST['valor']) || empty($_POST['valor'])):
		...
		echo "<script> x0p('Opss...', 'Escolha uma opção!', 'error', false);" . $soundEffect
		...
		$soundFXFile = $site . 'addcarrinho.mp3';
		$soundEffect = soundEffectManager($soundFXFile);
		echo "<script> x0p('Sucesso!', 'Seu Pedido foi Adicionado!', 'ok', false);" . $soundEffect . "
				$(\"#popuppedido_{$_POST['id-item']}\").modal('hide');
				$(\".obsitem\").val(\"\");
